...
export class ServiceService {
  ...
  async createService(
    ...
    try {
      ...
      console.log(
        ...
        JSON.stringify(newServiceData, null, 2),
      );
      ...
      if (files && files.length > 0) {
        ...
        if (service.images && service.images.length > 0) {
          try {
            ...
            console.error(
              '⚠️ Failed to delete old service images from Supabase. Proceeding with update:',
              deleteError,
            );
        ...
        try {
          ...
          console.log(
            '✅ New service images uploaded successfully:',
            finalImageUrls,
          );
        } catch (uploadError) {
    ...
    try {
      ...
      if (service.images && service.images.length > 0) {
        try {
          ...
          console.error(
            '❌ Failed to delete service images from Supabase:',
            deleteError,
          );
  ...
  async updateServiceByName(
    ...
    try {
      ...
      const updatedImages = [...(service.images || []), ...newImageUrls];
  ...
  async getServiceById(serviceId: string): Promise<any> {
    // ⬅️ تغيير نوع الإرجاع إلى 'any' أو إنشاء DTO مخصص
    try {
