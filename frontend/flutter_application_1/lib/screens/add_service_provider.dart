...
// 1. لتخزين الصور الجديدة التي تم اختيارها فقط (قبل رفعها)
  ...
  List<String> _existingImageUrls = [];    List<Map<String, dynamic>> _highlights = [];
  ...
  void _showLoadingSnackBar(String message, {bool isError = false}) {
    ...
    void initState() {
      ...
      if (widget.existingData != null) {
        ...
        _latitudeCtrl.text = (d["latitude"] ?? d["location"]?["latitude"])?.toString() ?? "";
        _longitudeCtrl.text = (d["longitude"] ?? d["location"]?["longitude"])?.toString() ?? "";

        _fullDescCtrl.text = d["fullDescription"] ?? d["additionalInfo"]?["description"] ?? d["shortDescription"] ?? "";
        _shortDescCtrl.text = d["fullDescription"] ?? d["additionalInfo"]?["description"] ?? d["shortDescription"] ?? "";
        ...
        if (d['images'] is List) {
        _existingImageUrls = List<String>.from(d["images"]?.cast<String>() ?? []);
        }

        final rawHighlights = d["highlights"] ?? d["additionalInfo"]?["highlights"];
 ...
 Future<void> _pickImage() async {
   ...
   Future<void> _save() async {
    ...
    if (_selectedCategory == null || _selectedCity == null || _priceType == null) {
      _showLoadingSnackBar('Please select Category, City, and Price Type.', isError: true);
      return;
    }

    // 💡 تعديل: فحص الصور يجب أن يشمل الملفات الجديدة (باستخدام _images)
    if (_images.isEmpty && _existingImageUrls.isEmpty) { 
      _showLoadingSnackBar('Please upload at least one image for the service.', isError: true);
      return;
    }
    ...
    try {
      ...
      if (companyName == null) {
        ...
        _showLoadingSnackBar('Could not retrieve company name. Please contact support.', isError: true);
      ...
      try {
        ...
        _showLoadingSnackBar('Error adding service: ${e.toString()}', isError: true);
      } finally {
        setState(() => _isLoading = false);
      }
    } catch (e) {
      ...
      _showLoadingSnackBar('An unexpected error occurred: ${e.toString()}', isError: true);
      setState(() => _isLoading = false);
        }
  }
